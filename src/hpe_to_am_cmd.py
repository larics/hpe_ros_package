...
class hpe2amcmd():
    ...
    def run_ctl(self, r, R):
        ...
        scaling_x = 0.01; scaling_y = 0.01; scaling_z = 0.01;

        # Generate pose_ref --> if DRY run do not generate cmd
        gen_uav = True
        if gen_uav:
            pos_ref = self.gen_uav_cmd(scaling_x, scaling_y, scaling_z)
            #print(type(pos_ref))
        ...
        if gen_arm: 
            ...
            vel_ref.twist.linear.x = self.a_cmd.x
            vel_ref.twist.linear.y = self.a_cmd.y   
            vel_ref.twist.linear.z = self.a_cmd.z
